...
public class HUD {
	...
	public float getDirAngle(Player player, long time)
	{
		int last_dir;
		float dir_angle;
		
		if(time < player.TURN_LENGTH)
		{
			last_dir = player.LastDirection;
			if(player.Direction == 3 && last_dir ==2)
			{
				last_dir = 4;
			}
			if(player.Direction == 2 && last_dir == 3)
			{
				last_dir = 5;
			}
			dir_angle = ((player.TURN_LENGTH - time) * player.dirangles[last_dir] + 
					time * player.dirangles[player.Direction]) / player.TURN_LENGTH;
		}
		else
		{
			dir_angle = player.dirangles[player.Direction];
		}
		return dir_angle;
	}
}
