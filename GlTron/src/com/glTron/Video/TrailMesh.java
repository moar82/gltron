...
public class TrailMesh {
	...
	private void bowGeometry()
	{
		...
		for(i = 0; i < 10; i++)
		{
			...
			storeVertex(new StoreVertexParameter(pvOffset, s, t, (fFloor * trail_height),
					(fTop * trail_height), DECAL_WIDTH, 0.0f));
		...
		storeVertex(new StoreVertexParameter(pvOffset, s, 1.0f, (0.2f * trail_height), ( 0.3f * trail_height),
				DECAL_WIDTH, 0.0f));
	...
	private void trailGeometry()
	{
		...
		for(i=0; i < TrailOffset; i++)
		{
			...
			{
				storeVertex(new StoreVertexParameter(curVertex, segs[i], 0.0f, 0.0f,
						trail_height, fsegLength, fTotalLength));
				storeColor(curVertex,MeshColourType.E_COLOUR_TRAIL);
				curVertex += 2;
			}
			
			storeVertex(new StoreVertexParameter(curVertex, segs[i], 1.0f, 0.0f, trail_height,
		...
		storeVertex(new StoreVertexParameter(curVertex, s, 0.0f, 0.0f, trail_height, fsegLength,
				fTotalLength));
		storeColor(curVertex, MeshColourType.E_COLOUR_TRAIL);
		curVertex += 2;
		
		storeVertex(new StoreVertexParameter(curVertex, s, 1.0f, 0.0f, trail_height, fsegLength,
				fTotalLength));
		storeColor(curVertex,MeshColourType.E_COLOUR_TRAIL );
		curVertex += 2;
	...
	private void storeVertex(
			StoreVertexParameter parameterObject)
	{
		
		Vec v;
		int texOffset = 2 * parameterObject.offset;
		
		int iNormal;
		float fUStart;
		
		if(parameterObject.s.vDirection.v[0] == 0.0f)
			iNormal = 0;
		else
			iNormal = 2;
		
		fUStart = (parameterObject.fTotalLength / DECAL_WIDTH) - (float)Math.floor(parameterObject.fTotalLength / DECAL_WIDTH);
		
		v = new Vec(
				 (parameterObject.s.vStart.v[0] + parameterObject.t * parameterObject.s.vDirection.v[0]),
				 ( parameterObject.s.vStart.v[1] + parameterObject.t * parameterObject.s.vDirection.v[1]),
				 parameterObject.fFloor);
...
//		Log.e("GLTRON", Debug);
		...
		TexCoords[texOffset] = fUStart + parameterObject.t * parameterObject.fSegLength / DECAL_WIDTH;
		TexCoords[texOffset + 1] = 0.0f;
		
		Vertices[parameterObject.offset * 3] = v.v[0];
		Vertices[(parameterObject.offset * 3) + 1] = v.v[1];
		Vertices[(parameterObject.offset * 3) + 2] = v.v[2];
		
		Normals[parameterObject.offset * 3] = normals[iNormal].v[0];
		Normals[(parameterObject.offset * 3) + 1] = normals[iNormal].v[1];
		Normals[(parameterObject.offset * 3) + 2] = normals[iNormal].v[2];
		
		texOffset += 2;
		v.v[2] = parameterObject.fTop;
		TexCoords[texOffset] =  fUStart + parameterObject.t * parameterObject.fSegLength / DECAL_WIDTH;
		TexCoords[texOffset + 1] = 1.0f;
		Vertices[(parameterObject.offset + 1) * 3] = v.v[0];
		Vertices[((parameterObject.offset + 1) * 3) + 1] = v.v[1];
		Vertices[((parameterObject.offset + 1) * 3) + 2] = v.v[2];
		
		Normals[(parameterObject.offset + 1) * 3] = normals[iNormal].v[0];
		Normals[((parameterObject.offset + 1) *3) + 1] = normals[iNormal].v[1];
		Normals[((parameterObject.offset + 1) *3) + 2] = normals[iNormal].v[2];
	}
